...
3. **Advise** — the agent turns the patterns into copy-pasteable fixes: `CLAUDE.md`
   rules, an encoded verify gate, a settings allowlist.

The heavy lifting is deterministic, so it's cheap; the agent only writes the
judgment calls.
...
- **Sessions** (`--mode tokens`) — where you waste tokens *and* where loops break
  ...
  runs, better loops.
- **Repo** (`--mode repo`) — structure that taxes every agent: junk, duplicates,
  orphans, god-files.
...
Full tables and method: [bench/README.md](bench/README.md).
